    ...
    <section ref={ref} className="px-md tablet:px-2xl py-3xl max-w-7xl mx-auto w-full">
      ...
      <div className="grid grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-3 gap-md">
        {CERTIFICATES.map((cert, i) => (
          <motion.div key={cert.titleKey}
            initial={{ opacity: 0, y: 24 }}
            animate={inView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.5, delay: i * 0.08, ease: [0.16, 1, 0.3, 1] }}
            whileHover={{ y: -3 }}
            className="rounded-2xl border border-[var(--color-border)] overflow-hidden group hover:shadow-md transition-all duration-300 bg-[var(--color-bg-primary)]"
          >
